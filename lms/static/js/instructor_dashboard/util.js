...
(function() {
    ...
    sentToFormatter = function(row, cell, value, columnDef, dataContext) {
        if (value === null) {
            return unknownP();
        } else {
    ...
    unknownIfNullFormatter = function(row, cell, value, columnDef, dataContext) {
        if (value === null) {
            return unknownP();
        } else {
    ...
    KeywordValidator = (function() {
        ...
        KeywordValidator.validate_string = function(string) {
            var found_keyword, found_keywords, invalidKeywords, isValid,
                keywords, regex_match, _fn, i, len;
            ...
            _fn = function(found_keyword) {
                if (__indexOf.call(keywords, found_keyword) < 0) {
                    return invalidKeywords.push(found_keyword);
                }
            };
            for (i = 0, len = found_keywords.length; i < len; i++) {
                found_keyword = found_keywords[i];
                _fn(found_keyword);
            }
