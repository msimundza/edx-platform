...
class SubsectionGradeFactory(object):
    ...
    def update(self, subsection, block_structure=None, only_if_higher=None):
        ...
        should_update_grade = True
        if only_if_higher:
            calculated_grade, should_update_grade = self._update_only_if_higher(
                subsection, block_structure, calculated_grade,
            )

        if should_update_grade:
            grade_model = calculated_grade.update_or_create_model(self.student)
            self._update_saved_subsection_grade(subsection.location, grade_model)

        return calculated_grade, should_update_grade

    def _update_only_if_higher(self, subsection, block_structure, new_calculated_grade):
        """
        Returns a tuple of the chosen grade and whether it needs to be persisted.
        """
        try:
            ...
            orig_subsection_grade = SubsectionGrade(subsection, self.course).init_from_model(
                self.student, grade_model, block_structure, self._scores_client, self._submissions_scores,
            )
            if new_calculated_grade.graded_total < orig_subsection_grade.graded_total:
                return orig_subsection_grade, False
        except PersistentSubsectionGrade.DoesNotExist:
            pass

        return new_calculated_grade, True
    ...
    def _get_bulk_cached_grade(self, subsection, block_structure):  # pylint: disable=unused-argument
        ...
        saved_subsection_grade = self._get_bulk_cached_subsection_grades(subsection.location)
    ...
    def _get_bulk_cached_subsection_grades(self, subsection_usage_key):
        """
