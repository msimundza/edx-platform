class @Sequence
  ...
  prepareSafari: ->
    console.log("Preping for safari")
    $.browser.chrome = /chrome/.test(navigator.userAgent.toLowerCase())
    if $.browser.chrome
      $.browser.safari = false
    if $.browser.safari
      $('.section-previous-square').css({ height: '24px', left: '26px', bottom: '-12px' })
      $('.section-next-square').css({ height: '24px', left: '-31px', bottom: '-12px' })
  updatePageTitle: ->
  ...
  updateCss: ->
    if @position == 1 
      $('.sequence-nav-button.button-previous').css({ left : "-15px" })
      $('.sequence-nav-button.button-next').css({ right : "-10px" })
    if @position == @contents.length 
      $('.sequence-nav-button.button-next').css({ right : "-18px" })
      $('.sequence-nav-button.button-previous').css({ left : "-10px" })
    if @position != 1 and @position != @contents.length 
      $('.sequence-nav-button.button-previous').css({ left : "-10px" })
      $('.sequence-nav-button.button-next').css({ right : "-10px" })
    if @position == 1 and @contents.length == 1
      $('.sequence-nav-button.button-next').css({ right : "-18px" })
      $('.sequence-nav-button.button-previous').css({ left : "-18px" })
